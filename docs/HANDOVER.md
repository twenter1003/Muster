...
- **현재 브랜치**: `main` (`064f1c8`, [PR #75](https://github.com/twenter1003/Muster/pull/75) 머지·배포 완료 — 훅 리포팅 로직 중복 정리 + 훅 버전 배너)

## 저장소 상태 — 훅 리포팅 로직 중복 정리 + 백필 정확도 개선 + 훅 버전 배너 (PR #75)

> **다음 세션 최우선 착수 과제는 이 섹션 맨 아래 "다음 세션 과제"를 볼 것.**

사용자 지적: "훅이 낡으면 매번 수동으로 고치는 게 비효율적이다, 실시간 모니터링과 과거
기록 백필이 실제 서비스로 동작해야 한다" — 당초 계획했던 "훅 버전 배너"만으로는 부족하다는
지적이었다. 코드를 다시 뜯어보니 토큰 사용량 리포팅 로직이 4곳에서 독립적으로
재구현돼 있었고, 그중 절반은 이미 틀린 채로 방치돼 있었다.

- ✅ **`apps/web/public/connect.mjs` 삭제**: 캐시 토큰 분리(DESIGN_DRIFT 18) *이전*
  버전의 훅 전체가 통째로 박제된 죽은 사본. `GET /api/v1/connect.mjs`(신규 유저가
  실행하는 `curl | node -`가 받는 스크립트)를 서빙하는 `ConnectScriptController`가
  경로 8개를 순서대로 추측하다 실패하면 이 파일로 폴백했다 — 지금은 안 걸리지만
  배포 구조가 바뀌면 신규 유저 전원이 옛 훅을 받는 사고로 이어질 수 있었다. 경로를
  실제로 cwd가 갈리는 두 경우(프로덕션 `/app`, 로컬·테스트 `apps/api`)만 남겼다.
- ✅ **`scripts/backfill-claude-tokens.mjs` 삭제**: 별도의 세 번째 파서를 쓰는 개인용
  일회성 스크립트. **프로덕션 Supabase DB 비밀번호가 평문으로 커밋돼 있었다**(이
  저장소는 GitHub 퍼블릭). 사용자가 이번 세션에서 Supabase 콘솔에서 비밀번호를
  로테이션했고, `migration:show`로 새 비밀번호의 연결을 확인한 뒤 마이그레이션·배포를
  진행했다.
- ✅ **백필 파서 교체**: `scanClaudeSessions`/`scanAntigravitySessions`가 토큰을 합계
  하나로만 넣고 모델명도 안 넣는 옛날 파서를 따로 쓰고 있었다. 실시간 훅과 같은
  파서(`sumUsageFromTranscript`, `sumUsageFromStepsDb`)를 재사용하도록 교체 —
  프로젝트를 새로 가져와도 과거 세션이 모델별·캐시 내역까지 정확히 채워진다.
- ✅ **CI에 임베딩 훅 동기화 확인 추가**: `sync-embedded-hooks.mjs --check`를 새
  `scripts` 잡으로 추가. 훅을 고치고 `muster-connect.mjs` 안의 base64 사본 갱신을
  깜빡하는 걸 사람이 아니라 빌드가 막는다.
- ✅ **훅 버전 리포팅 + 배너**: 훅이 자기 코드 버전(`HOOK_VERSION`, 두 훅 모두 `1`)을
  같이 보고하고, 서버(`LATEST_HOOK_VERSION`, `hook-version.ts`)가 최신보다 낮은 걸
  감지하면 프로젝트 상세 화면 토큰 카드 상단에 "이 머신 훅이 오래됨" 배너를 띄운다.
  이번 세션에서 실제로 겪은 사고(리포 코드는 고쳐졌는데 이 컴퓨터에 설치된 훅만 218줄
  뒤처져 모델·토큰 필드를 아예 안 보내고 있었던 것)를 다음에는 DB를 직접 뒤지지
  않고도 화면에서 바로 알 수 있게 한다.
- ✅ **스키마 변경**: `agent_runs.hook_version` 컬럼 추가(nullable, migration
  `1788940000000-AddAgentRunHookVersion`). 로컬 Postgres에서 up/down/up 왕복 확인 후
  프로덕션 적용 완료(`migration:show` 17개 전부 `[X]`).
- ✅ **검증**: 단위 766개(신규 백필 파서 테스트 7개 + 프론트 `hookVersion` 테스트 5개
  포함) + e2e 243개 전부 통과, 타입체크·린트·포맷 통과, 목 서버로 배너 노출/미노출
  양쪽 시각 확인.
- ✅ **배포**: `064f1c8` 배포, 리비전 `muster-00045-5df`가 트래픽 100% 서빙 중.
  헬스체크 200 확인.

### 다음 세션 과제

1. **다른 컴퓨터 훅 확인**: 이 컴퓨터 훅은 지난 세션에서 갱신했지만, 다른 컴퓨터가
   있다면 그쪽 훅도 낡았을 가능성이 크다 — 이번 PR의 배너가 화면에서 알려줄 것이므로
   프로젝트 상세 화면에서 확인하고, 뜨면 `npx muster-connect` 재실행 안내.
2. **Supabase DB 비밀번호 로테이션 마무리**: 비밀번호 자체는 로테이션·검증·적용까지
   끝났다. `scripts/backfill-claude-tokens.mjs`가 git 히스토리에 남긴 옛 비밀번호는
   이미 무효화됐으므로 히스토리 스크럽은 급하지 않지만, 완전히 정리하려면 별도로
   고려할 것(BFG/git-filter-repo, 퍼블릭 레포라 poisoned 상태는 계속 남는다).
3. **그다음 후보**: 상세 페이지 재편(개요/에이전트/배포/설정 탭 구조) — 이전
   HANDOVER 스냅샷 참조. 신규 기능은 여전히 금지, 개선 작업만 허용.
