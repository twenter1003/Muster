...
- **목표 저장 직후 진행률 자동 분석**: 지금은 목표를 저장해도 진행률(%·바·요약)이 바로
  안 뜨고, 사용자가 "다시 분석"을 한 번 더 눌러야 나온다. 사용자 피드백: 저장 직후
  자동으로 분석까지 이어지면 좋겠다는 것. 웹훅마다 자동 재분석은 비용 때문에 일부러
  안 하기로 했지만(DESIGN_DRIFT §12), "사람이 막 저장한 직후" 1회는 웹훅과 성격이 달라
  붙여도 비용 문제가 없다는 게 결론이었음 — 구현만 안 됨.
- **진행률 "다시 분석" 버튼의 최초 상태 라벨**: 한 번도 분석한 적 없을 때도 버튼이
  "다시 분석"이라 문구가 어색하다(바로 위에 "아직 분석하지 않았다"라고 써 있는데 버튼은
  "다시"). `progress`가 null이면 "분석하기", 있으면 "다시 분석"으로 갈라야 함.
  (`apps/web/src/routes/ProjectDetailPage.tsx`의 `GoalsProgressCard`)
