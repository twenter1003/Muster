...
- **타 개발자 온보딩용 1줄 연동 CLI (`npx muster-connect`) 및 멀티 모델 토큰 관제**:
  - Claude Code 뿐만 아니라 Antigravity 세션 토큰 사용량도 자동 수집.
  - 사용자에 따라 1개 회사의 모델만 사용할 수 있으므로, 연동 시 수집 대상(Claude Code, Antigravity)을 옵션화(선택 가능)하고 대시보드에서 보고 싶은 회사의 LLM 토큰 사용량만 필터링/선택해서 볼 수 있도록 옵션 제공.
